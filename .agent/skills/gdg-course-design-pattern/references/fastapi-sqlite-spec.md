...
10. Optionally compress old uncompressed turns based on compression thresholds.
...
- Persist summary in `compressions` and optional `messages` record with role `system`.
- Mark compressed raw messages as `compressed = 1` but keep raw data for auditability.

## Concurrency And Transactions
- Message, embedding, and compression writes are each executed in their own short SQLite transaction.
- Partial persistence is possible if a later phase fails; API error handling must remain deterministic and auditable.
